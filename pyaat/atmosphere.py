...
class atmosISA(object):
    ...
    """
    def __init__(self, deltaT = 288.15):
        self.deltaT = deltaT
        self._altitude = 0           # altitude [m]     
        self._parameters = array(
            [[0., 1524., 3048., 4572., 6096., 7620., 9144., 10668., 12192., 20063, 32162, 47350, 51413, 71802, 86000],                    # geometric altitude [m]
             [288.15, 278.25, 268.35, 258.45, 248.55, 238.65, 228.75, 218.85, 216.65, 216.64, 228.65, 270.65, 270.65, 214.65, 186.87],          # base atmospheric temperature [K]
             [101300., 84300., 69700, 57200, 46600, 37600, 30100, 23800, 18800, 5474.9, 868.02, 110.91, 66.939, 3.9564, 0.3734]])      # base atmospheric pressure [Pa]
    ...
    def _rho(self): # air density [kg/m³]
        return self._pressure / (R_AIR * (self._temperature - 288.15 + self.deltaT))
        
    @property
    def _soundVel(self):  # velocity of sound [m/s]
        return sqrt(GAMMA_AIR * R_AIR * (self._temperature - 288.15 + self.deltaT))
