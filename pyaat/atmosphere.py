...
class metaAtmosphere(ABC):
    """
    Metaclass for the atmosphere models. The interface for all models is exactly the same.
    """
    def __init__(self, deltaT = 0.0, altitude = 0.0):
    ...
    def set_altitude(self, entry):
        if entry <= self._maxAltitude:
            self._altitude = entry
        else:
            self._altitude = self._maxAltitude
            self._flagError = 'INVALID'
            self._error = 'Altitude out of range (atmosphere model)'

    def get_altitude(self):
        return self._altitude

    def set_deltaT(self, entry):
        self._deltaT = entry
...
class atmosCOESA(metaAtmosphere):
    """
    ...
    def __init__(self, deltaT=0, altitude=0):
        super().__init__(deltaT, altitude)
        self._maxAltitude = 86000.0
    ...
    @property 
    def _pressure(self): # pressure [Pa]      
        return interp(self._altitude, self._parameters[0], self._parameters[2])*1e2
...
class atmosISA(metaAtmosphere):
    ...
    def __init__(self, deltaT=0.0, altitude=0.0):
        ...
        self._parameters = array(
            [[0., 1524., 3048., 4572., 6096., 7620., 9144., 10668.,
             12192., 20063, 32162, 47350, 51413, 71802, 86000],                    # geometric altitude [m]
             [288.15, 278.25, 268.35, 258.45, 248.55, 238.65, 228.75, 218.85,
             216.65, 216.64, 228.65, 270.65, 270.65, 214.65, 186.87],          # base atmospheric temperature [K]
             [101300., 84300., 69700, 57200, 46600, 37600, 30100, 23800,
             18800, 5474.9, 868.02, 110.91, 66.939, 3.9564, 0.3734]])      # base atmospheric pressure [Pa]
...
class CookModel(metaAtmosphere):
    """
    ...
    @property
    def _soundVel(self):  # velocity of sound [m/s]
        return SOUND_VEL_SEA

class seaLevel(metaAtmosphere):
    """
...
class windTunnel(metaAtmosphere):
